...
int main(void)
{
	...
	while (1) {
		...
		case 0:	// mkdir	
			sp = strtok(NULL, " ");
			if (sp != NULL)		my_mkdir(sp);
			else 				printf("mkdir error\n");
			break;
		case 1: // rmdir
			sp = strtok(NULL, " ");
			if (sp != NULL) 	my_rmdir(sp);
			else 				printf("rmdir error\n");
			break;
		case 2: // ls
			my_ls();
			break;
		case 3: // cd
			sp = strtok(NULL, " ");
			if (sp != NULL)		my_cd(sp);
			else				printf("cd error\n");
			break;
		case 4: // create
			sp = strtok(NULL, " ");
			if (sp != NULL)     my_create(sp);
			else                printf("create error\n");
			break;
		case 5: // rm
			sp = strtok(NULL, " ");
			if (sp != NULL)     my_rm(sp);
			else                printf("rm error\n");
			break;
		case 6: // open
			sp = strtok(NULL, " ");
			if (sp != NULL)		my_open(sp);
